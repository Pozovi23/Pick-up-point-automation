...
def graph():
    ...
    with open(test_csv, "w") as f:
        f.write("""previous,from,to,direction
...
def test_find_shortest_path(graph):
    assert graph.find_shortest_path('base', 'Shelf_1') == ['base', 'Crossroad_1', 'Crossroad_2', 'Shelf_1']
    assert graph.find_shortest_path('Crossroad_1', 'Shelf_1') == ['Crossroad_1', 'Crossroad_2', 'Shelf_1']
    assert graph.find_shortest_path('Shelf_1', 'base') is None  # No reverse path in this test graph